...
def compare_state_marks(json1_records: List[NormalizedRecord], 
    ...
    for rec2 in json2_records:
        ...
        match_errors = []
        if rec1.owner_norm != rec2.owner_norm: match_errors.append(f"Owner: [J1: '{rec1.owner_raw}'] != [J2: '{rec2.owner_raw}']")
        if rec1.goods_norm != rec2.goods_norm: match_errors.append(f"Goods: [J1: '{rec1.goods_raw}'] != [J2: '{rec2.goods_raw}']")
        if rec1.trademark_norm != rec2.trademark_norm: match_errors.append(f"Trademark: [J1: '{rec1.trademark_raw}'] != [J2: '{rec2.trademark_raw}']")
        ...
        if not match_errors:
            passed += 1
            report_lines.append(f"PASS: {rec_id} (Reg: {reg_no})")
        else:
...
def compare_common_law(json1_records: List[NormalizedRecord], 
    ...
    for rec2 in json2_records:
        ...
        if rec1:
            ...
            if not match_errors:
                passed += 1
                report_lines.append(f"PASS: {rec_id} (Owner: {rec2.owner_raw})")
            else:
...
def compare_web_common_law(json1_records: List[NormalizedRecord], 
    ...
    for rec2 in json2_records:
        ...
        if rec1:
            match_errors = []
            # Only compare goods if JSON 2 has them (Preserving Pattern 3 semantics)
            if rec2.goods_raw and rec1.goods_norm != rec2.goods_norm:
                match_errors.append(f"Goods: [J1: '{rec1.goods_raw}'] != [J2: '{rec2.goods_raw}']")
            if rec1.owner_norm != rec2.owner_norm:
                match_errors.append(f"Owner: [J1: '{rec1.owner_raw}'] != [J2: '{rec2.owner_raw}']")
            if rec1.class_list and rec2.class_list and rec1.class_list != rec2.class_list:
                match_errors.append(f"Class: {rec1.class_list} != {rec2.class_list}")
            if rec1.trademark_norm != rec2.trademark_norm:
                match_errors.append(f"Trademark: [J1: '{rec1.trademark_raw}'] != [J2: '{rec2.trademark_raw}']")
                
            if not match_errors:
                passed += 1
                report_lines.append(f"PASS: {rec_id} (Mark: {rec2.trademark_raw})")
            else:
...
def compare_web_domain(json1_records: List[NormalizedRecord], 
    ...
    total_effective = 0
    failures = []
    report_lines = ["CATEGORY: WEB_DOMAIN GATE 2 REPORT", "="*40]
    ...
    for rec2 in json2_records:
        ...
        if rec1:
            ...
            if not match_errors:
                passed += 1
                report_lines.append(f"PASS: {rec_id} (TM: {tm2})")
            else:
    ...
    return CategoryComparisonResult("WEB_DOMAIN", passed, total_effective, failures, report_lines)
